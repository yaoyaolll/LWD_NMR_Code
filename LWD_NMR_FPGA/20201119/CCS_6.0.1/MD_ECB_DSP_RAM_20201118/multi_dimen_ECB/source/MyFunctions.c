...
void McbspSendData(Uint32 StartAddr,Uint32 DataNum)       //Mcbsp发送数据
{
	...
	for(SendCnt=0;SendCnt<DataNum;SendCnt++)  
    {
		...
		{ ; }
		McbspaRegs.DXR1.all=*SendTempPt;     //发送数据 
		SendTempPt++;                        //指针指向下一个数据
    }      
...
{   
	...
	NSTATE_DATA			= NoiseAcqTim*FPGA_COUNT;	//装载信号：噪声采集时间的FPGA计数值
	NSTATE_LOAD_EN		= USER_ENABLE;//使能噪声状态机参数装载信号
	NSTATE_LOAD_DIS		= USER_DISABLE;	//清零噪声状态机参数装载信号

	NoiseDivFreq	= 1000/NoiseAcqFreq;
	NoiseDivNum		= (NoiseDivFreq/2)*33+NoiseDivFreq%2;    //噪声分频系数
	NoiseAcqNum		= NoiseAcqTim*NoiseAcqFreq/10;        //噪声采样点数
...
//Acuire noise data once
	NTimeDec = (ScaleTe*50-90)*FPGA_COUNT;
...
//Scan once
	...
	{
		FreqScan = MiniFreq + MiniFreqCnt*StepFreq;
...
{
	PulseGainIQ	= _IQ(PulseGainAry[PulseGain]);      // PluseGain = 140 Gain = 890
...
{		
	...
	*(Uint16 *)DDS_FREQ_ADDR	= DCWorkFreq;
	...
	SIGNAL_LOAD_DIS	= USER_DISABLE;	          
    //90度激励时间计数值 
	Pulse90Width	= NintyWidthAry[FreqCode]* FPGA_COUNT;     
	//180度激励时间计数值
