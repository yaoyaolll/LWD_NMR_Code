...
#define CAL_TABLE_LEN  19
#define WELL_TABLE_LEN 57
...
void DownloadTable(Uint16 DownDataBuf)
{
	...
	else if (DownloadTableCnt == 1 && DownTableFlag == SET) // 参数表长度
	{
		...
		if (DownDataBuf == CAL_TABLE_LEN+2)
		{
			SaveTablePt = (Uint16 *)0x8002;
			*SaveTablePt++	= DownDataBuf;
			DownloadTableCnt = 2;
		}
		else if (DownDataBuf == CAL_TABLE_LEN+2)
		{
	...
	else if (DownloadTableCnt == ParamTableLen+1 && DownTableFlag == SET)   // 最后一个数据为CheckSum
	{
		Uint16 check_sum = 0;
		int i=1;
