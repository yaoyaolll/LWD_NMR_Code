...
void PPModeTop(void)
{
	...
	Tel	= Tes;
	Ne	= PPM_NE_1A;
	Pulse90StoreAddr	= PPM_TABLE_START+(Uint32)DataTotalNum+19;
	PulseF180StoreAddr	= Pulse90StoreAddr+EchoNum;
	PulseL180StoreAddr	= PulseF180StoreAddr+EchoNum;
	EchoStorAddr		= PPM_TABLE_START+(Uint32)19;
	...
	*SaveNTempPt= CenterFreq;   // 工作频率
	...
	SaveNTempPt = (int *)(PPM_TABLE_START + 15);
	*SaveNTempPt++ = 0;          		// Q值
	...
	*SaveNTempPt = CheckSum;
	...
	SaveNTempPt	= (int *)(PPM_TABLE_START+3);
	SaveSTempPt	= (Uint16 *)(PPM_TABLE_START+6);
	StoreMiniAryPt	= &PPModeMiniNumAry;
	StoreMini(1,SaveNTempPt,SaveSTempPt);
	
	SciaSendDataNWords(PPM_TABLE_START, DataTotalNum+407);
	ChangePhase();
	return;
}
