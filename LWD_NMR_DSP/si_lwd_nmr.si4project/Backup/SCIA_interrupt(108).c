...
// 发送错误帧
void ErrorSendData(Uint16 err, Uint16 lastCheckSum)
{
	Uint16 CheckSum = 0;
	GpioDataRegs.GPFDAT.bit.GPIOF9 = 1;    // SCIA设置为发送状态
	
	while (SciaTx_Ready() == 0) {}
	SciaRegs.SCITXBUF = 0x9996;            // 发送错误帧头
	
	while (SciaTx_Ready() == 0) {}
	SciaRegs.SCITXBUF = err;               // 发送错误数据

	while (SciaTx_Ready() == 0) {}
	SciaRegs.SCITXBUF = lastCheckSum;      // 发送错误数据
...
interrupt void SCIRXINTA_ISR(void)     // SCI-A接收中断函数
{
	...
	if (SciaRecFlag)
	{
		if(BufferSciaDataAll==0x1328 || DownTableFlag==SET)             //下载参数表命令
		{
			DownloadTable(BufferSciaDataAll);
		}
		else if(BufferSciaDataAll==0x1428 || RecSendTableFlag==SET)     //上传参数表命令
		{
