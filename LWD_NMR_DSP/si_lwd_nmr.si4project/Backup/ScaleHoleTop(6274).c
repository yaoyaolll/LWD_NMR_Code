...
void ScaleHoleModeTop(void)
{
	...
	ToMSNoiseAddr	= SCALETABLE_START + 3;
	ToMSSignalAddr	= SCALETABLE_START + 6;
	EchoStorAddr		= SCALETABLE_START+19;
	Pulse90StoreAddr	= SCALETABLE_START+2*Ne+19;
	PulseF180StoreAddr	= SCALETABLE_START+2*Ne+20;
	PulseL180StoreAddr	= SCALETABLE_START+2*Ne+21;
	...
	{    
		...
		*SaveNTempPt = CenterFreq * 10; 	// 中心频率
		...
		SaveNTempPt = (int *)(SCALETABLE_START + 15);
		*SaveNTempPt++ = 0;          		// Q值
		...
		*SaveNTempPt = CheckSum;
		...
		SciaSendDataNWords(SCALETABLE_START, 2*Ne+23);    //上传刻度结果
	}     
	else    // 孔隙度刻度与校验模式
	{
		...
		*SaveNTempPt = CenterFreq * 10; 	// 中心频率
		...
		SaveNTempPt = (int *)(HOLETABLE_START + 15);
		*SaveNTempPt++ = 0;          		// Q值
		*SaveNTempPt++ = 0x294;				// 参考幅值
		*SaveNTempPt++ = Width90Pulse;      // 90度脉冲宽度
		SavePhaseWord();       // 存储相位
		...
		*SaveNTempPt = CheckSum;
		...
		SciaSendDataNWords(HOLETABLE_START, 2*Ne+23);    //上传刻度结果
	}             
	ChangePhase();
}
/*-----------------------------------------------------------------------------------------*/
