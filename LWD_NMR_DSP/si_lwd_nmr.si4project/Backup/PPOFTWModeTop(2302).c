...
void PPOFTWModeTop(void)
{
	...
	Pulse90StoreAddr	= PPOFTW_TABLE_START+(Uint32)DataTotalNum+19;
	PulseF180StoreAddr	= Pulse90StoreAddr+EchoNum;
	PulseL180StoreAddr	= PulseF180StoreAddr+EchoNum;
	EchoStorAddr		= PPOFTW_TABLE_START+(Uint32)19;
	...
	*SaveNTempPt = CheckSum;
	...
	SaveNTempPt	= (int *)(PPOFTW_TABLE_START+3);
	SaveSTempPt	= (Uint16 *)(PPOFTW_TABLE_START+6);
	StoreMiniAryPt	= &PPOFTWMiniNumAry;
	StoreMini(1,SaveNTempPt,SaveSTempPt);
	
	SciaSendDataNWords(PPOFTW_TABLE_START, DataTotalNum+338);
	ChangePhase();
	return;
}
