...
void PPDIFModeTop(void)
{
	...
	Tel	= Tes;
	Ne	= PPDIF_NE_1A1B;
	Pulse90StoreAddr	= PPDIF_TABLE_START+(Uint32)DataTotalNum+19;
	PulseF180StoreAddr	= Pulse90StoreAddr+EchoNum;
	PulseL180StoreAddr	= PulseF180StoreAddr+EchoNum;
	EchoStorAddr		= PPDIF_TABLE_START+(Uint32)19;
	...
	*SaveNTempPt++ = CenterFreq;   		// 工作频率
	...
	SaveNTempPt = (int *)(PPDIF_TABLE_START + 15);
	*SaveNTempPt++ = 0;          		// Q值
	...
	*SaveNTempPt = CheckSum;
	...
	SaveNTempPt	= (int *)(PPDIF_TABLE_START+3);
	SaveSTempPt	= (Uint16 *)(PPDIF_TABLE_START+6);
	StoreMiniAryPt	= &PPDIFMiniNumAry;
	StoreMini(1,SaveNTempPt,SaveSTempPt);
	
	SciaSendDataNWords(PPDIF_TABLE_START, DataTotalNum+410);
	ChangePhase();
	return;
}
