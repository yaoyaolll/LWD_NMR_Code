...
{      
	...
	for(MiniFreqCnt=0;MiniFreqCnt<9;MiniFreqCnt++)//某频带已扫频个数0-8
	{
		...
		SaveNTempPt	= (int *)(SCANTABLE_START + MiniFreqCnt*3 + 3);    //指向均值地址
		...
		*(Uint16 *)DDS_FREQ_ADDR	= FreqScan;
		...
		SaveSTempPt = (Uint16 *)(SCANTABLE_START + MiniFreqCnt + 30);    //指向幅值地址
	...
	*SaveNTempPt	= CenterFreq * 10;    // 中心频率，下发和上传的中心频率单位是0.1kHz
	...
	SaveNTempPt = (int *)(SCANTABLE_START + 39);
	*SaveNTempPt++ = 0;          		// Q值
	*SaveNTempPt++ = 0x294;				// 参考幅值
	...
	SaveNTempPt = (int *)(SCANTABLE_START);
	int i=0;
	for (;i<SCAN_MODE_DATA_FRAME_LEN-2;++i)  // CheckSum计算前面的数据时，除去CheckSum本身和0xAA55
	{
	...
	*SaveNTempPt = CheckSum;
     ...
     //上传数据
	SciaSendDataNWords(SCANTABLE_START, SCAN_MODE_DATA_FRAME_LEN-1);  // 0xAA55在此函数中自动附加
	return;
}
