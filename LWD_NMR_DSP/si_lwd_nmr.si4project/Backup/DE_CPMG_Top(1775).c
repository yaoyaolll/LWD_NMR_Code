...
{      
	...
	MiniScan(CenterFreq, DECPMGTABLE_START+3, DECPMGTABLE_START+2*Ne+6);   

	Tes	= (Uint32)100 * STWTE_TE * FPGA_COUNT;        //短TE值 TE 0.1ms
	Tel	= Tes;                                        //将SINGLE_TE转换为10ns 单位即FPGA计数值
	Pulse90StoreAddr	= DECPMGTABLE_START+2*Ne+19;  //计算90°脉冲的幅值地址pluse90addr
	PulseF180StoreAddr	= DECPMGTABLE_START+2*Ne+20;  //计算首个180°脉冲的幅值地址pluse180addr1
	PulseL180StoreAddr	= DECPMGTABLE_START+2*Ne+21;  //计算末个180°脉冲的幅值地址pluse180addr2
	EchoStorAddr		= DECPMGTABLE_START+19;       //计算回波数据的首地址echostraddr
	...
	*SaveNTempPt= CenterFreq;      		// 工作频率
	...
	SaveNTempPt = (int *)(DECPMGTABLE_START + 15);
	*SaveNTempPt++ = 0;          		// Q值
	...
	*SaveNTempPt = CheckSum;
	...
	//上传数据
	SciaSendDataNWords(DECPMGTABLE_START, 2*Ne+23);
