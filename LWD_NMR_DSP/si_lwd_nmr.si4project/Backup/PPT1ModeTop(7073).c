...
void PPT1ModeTop(void)
{
	...
	Tel	= Tes;
	Ne	= PPT1_NE_1A;
	Pulse90StoreAddr	= PPT1_TABLE_START+(Uint32)DataTotalNum+19;
	PulseF180StoreAddr	= Pulse90StoreAddr+EchoNum;
	PulseL180StoreAddr	= PulseF180StoreAddr+EchoNum;
	EchoStorAddr		= PPT1_TABLE_START+(Uint32)19;
	...
	*SaveNTempPt++ = CenterFreq;   		// 工作频率
	...
	SaveNTempPt = (int *)(PPT1_TABLE_START + 15);
	*SaveNTempPt++ = 0;          		// Q值
	...
	*SaveNTempPt = CheckSum;
	...
	SaveNTempPt	= (int *)(PPT1_TABLE_START+3);
	SaveSTempPt	= (Uint16 *)(PPT1_TABLE_START+6);
	StoreMiniAryPt	= &PPT1MiniNumAry;
	StoreMini(1,SaveNTempPt,SaveSTempPt);
	
	SciaSendDataNWords(PPT1_TABLE_START, DataTotalNum+413);
	ChangePhase();
	return;
}
