...
{    
	...
	RELAY_ON_CLOSE = RelayAry[1];   //继电器闭合
	Delay(120);                       
	...
	*SaveNTempPt++ = PulseCalNum+19;  	// 长度
	*SaveNTempPt++ = 0x000B;  			// 工作模式
	*SaveNTempPt = CenterFreq*10;		// 中心频率

	SaveNTempPt = (int *)PLUSETABLE_START + 16;
	*SaveNTempPt++ = 0;          		// Q值
	...
	*SaveNTempPt = CheckSum;
	...
	modeDataSendLen = PulseCalNum+20;
...
void PulseSave(void)    // 发射信号波形采集检测激励存储函数
{
	...
	SaveSTempPt = (Uint16 *)(PLUSETABLE_START+19);    //指向幅值地址
